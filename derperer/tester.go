...
func newTester(ctx context.Context, logf logger.Logf, latencyLimit time.Duration) (*tester, error) {
...
func (t *tester) Test(derpMap *tailcfg.DERPMap) *tailcfg.DERPMap {
	...
	wg.Add(len(derpMap.Regions) * 3)
	for _, region := range derpMap.Regions {
		go func(region *tailcfg.DERPRegion) {
			defer wg.Done()
			latency, err := t.measureICMPLatency(t.ctx, region, t.Pinger)
			if err != nil {
				zap.L().Debug("ICMP Error", zap.Int("regionID", region.RegionID), zap.Error(err))
				delete(newDerpMap.Regions, region.RegionID)
			} else if latency > t.latencyLimit {
				zap.L().Debug("ICMP Latency", zap.Int("regionID", region.RegionID), zap.Duration("latency", latency))
				delete(newDerpMap.Regions, region.RegionID)
			}
		}(region)
		go func(region *tailcfg.DERPRegion) {
			...
			if err != nil {
				zap.L().Debug("HTTPS Error", zap.Int("regionID", region.RegionID), zap.Error(err))
				delete(newDerpMap.Regions, region.RegionID)
			} else if latency > t.latencyLimit {
				zap.L().Debug("HTTPS Latency", zap.Int("regionID", region.RegionID), zap.Duration("latency", latency))
				delete(newDerpMap.Regions, region.RegionID)
			}
		}(region)
		go func(region *tailcfg.DERPRegion) {
			defer wg.Done()
			wg.Add(len(region.Nodes))
			for _, node := range region.Nodes {
				go func(node *tailcfg.DERPNode) {
					defer wg.Done()
					if err := t.testDerpNode(t.ctx, node, false); err != nil {
						delete(newDerpMap.Regions, region.RegionID)
					}
				}(node)
			}
